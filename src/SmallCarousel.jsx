...
export default function SmallCarousel() {
  return (
    <div className="w-full overflow-hidden mt-6">
      <div className="carousel-track flex gap-4" style={{ animation: 'scroll 350s linear infinite'}}>
        ...
        {duplicatedSlides.map((slideItem, idx) => (
          <div key={idx} className="w-118 h-70 flex-shrink-0 relative">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent text-white">
              <h3 className="text-lg font-semibold">{slideItem.title}</h3>
              <p className="text-sm text-gray-200">{slideItem.category}</p>
              <button className="mt-2 px-3 py-1 bg-white text-black text-sm font-medium rounded-full hover:opacity-90">
